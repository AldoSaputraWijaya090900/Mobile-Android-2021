...
public class DetilActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra("MAHASISWA")){
            ...
            etEmail.setText(mhs.getEmail()); etNomorHP.setText(mhs.getNomorhp()); etNim.setEnabled(false);
        }
        else{
    ...
    public void simpanData(View view){
        ...
        if(mNIM.length() <= 0 || mNama.length() <= 0 || mEmail.length() <= 0 || mNoHp.length() <= 0){
            ...
            Intent intentJawab = new Intent(); Mahasiswa mhs = new Mahasiswa(mNIM, mNama, mEmail, mNoHp); intentJawab.putExtra("MAHASISWA",mhs);
